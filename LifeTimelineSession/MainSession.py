...
class MainSession(DTSession.DTMainSession):
	...
	def loadData(self):
		...
		try:
			self.lifespan=int(Symmetric_Decrypt(self.password(),self.UserSetting().value("lifespan")))
			if self.lifespan==0:
				self.lifespan=150
		except:
			self.lifespan=150
		...
		try:
			birthday=Symmetric_Decrypt(self.password(),self.UserSetting().value("birthday"))
			self.birthday=QDate().fromString(birthday,"yyyyMMdd")
		except:
			self.birthday=QDate(1970,1,1)

		try:
			self.cubewidth=int(Symmetric_Decrypt(self.password(),self.UserSetting().value("cubewidth")))
			if self.cubewidth==0:
				self.cubewidth=20
		except:
			self.cubewidth=20
	...
	def saveData(self):
		...
		Symmetric_Encrypt_Save(self.password(),self.data,os.path.join(self.app.DataDir(),"LifeTimelime.dlcw"))
	
	def saveAllEncryptData(self):
		super().saveAllEncryptData()
		self.saveData()
		self.UserSetting().setValue("lifespan",Symmetric_Encrypt(self.password(),self.lifespan))
		self.UserSetting().setValue("birthday",Symmetric_Encrypt(self.password(),self.birthday.toString("yyyyMMdd")))
		self.UserSetting().setValue("cubewidth",Symmetric_Encrypt(self.password(),self.cubewidth))
